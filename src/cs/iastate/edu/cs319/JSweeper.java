...
/**
 * This class runs the game and contains the main function, it also acts as an administrative
 * controller for the entire game. Smaller tasks like updating and maintaining the JMinesweeperModel
 * and JMinefield are delegated to the JMinefieldController.
 *
...
public class JSweeper extends JFrame implements GameSettingsListener
{
    ...
    public static void main(String[] args)
    {
        EventQueue.invokeLater(new Runnable()
        {
            ...
            public void run()
            {
                try
                {
                    final JSweeper frame = new JSweeper("JSweeper");
    ...
    /**
     * Overloaded constructor. Takes a game title and then builds a new frame based on beginner
     * difficulty.
     *
     * @param title
     */
    ...
    /**
     * Overloaded constructor. Takes in a title and a difficulty and builds the game based on the
     * difficulty's built in parameters. Exits if difficulty is custom.
     *
     * @param title
     * @param difficulty
     */
    ...
    {
        this(title, difficulty, difficulty.getRows(), difficulty.getCols(), difficulty.getMines());
    ...
    /**
     * Default constructor. Takes in a title, difficulty, rows, columns, and mines. It first builds
     * the JMinefieldController, which then builds the model and JMinefield view, then this builds
     * the rest of the view and displays the frame.
     *
     * @param title
     * @param difficulty
     * @param rows
     ...
     * @param mines
     */
    public JSweeper(String title, Difficulty difficulty, int rows, int cols, int mines)
    {
        super(title);
        // set up the frame
        setBounds(0, 0, 0, 0);
     ...
     * Helper function that builds the JMinefieldController
     */
    private void buildController(Difficulty difficulty, int rows, int cols, int mines)
    {
        minefieldController = new JMinefieldController(difficulty, rows, cols, mines);
    }
    ...
    private void buildUI()
    {
        ...
        final JFaceButton faceButton = view.getFace();
        ...
        final JFlagCounter flagCounter = view.getFlagCounter();
        flagCounter.updateTextNumeric(model.getMines());
        minefieldController.addFlagCounterListener(flagCounter);
    }

    /*
     * Function that fires when the model's difficulty has changed. This sets up a new game with the
     * new difficulty.
     */
    @Override
    public void difficultyChanged()
    {
        ...
        final Difficulty diff = model.getDifficulty();
        ...
        if (diff == Difficulty.Custom)
        {
            ...
            final int mines = model.getMines();
